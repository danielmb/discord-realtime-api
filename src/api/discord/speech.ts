...
import { OpenAIConversationalAI } from '../openai/conversationClient.js';
import { resample16kTo24kMono } from '../../utils/audio.js';
...
class SpeechHandler {
  ...
  private client: OpenAIConversationalAI;
  private decoder: opus.OpusEncoder;
  private connection: VoiceConnection;

  constructor(
    client: OpenAIConversationalAI,
    ...
    try {
      ...
      this.connection.on('stateChange', (oldState, newState) => {
        if (
          ...
          logger.info(
            'Voice connection disconnected or destroyed. Cleaning up.',
          );
          this.cleanup();
        }
      });
   ...
   */
  private async createUserAudioStream(
    userId: string,
    connection: VoiceConnection,
  ): Promise<void> {
    try {
      const opusAudioStream: AudioReceiveStream = connection.receiver.subscribe(
        userId,
        {
      ...
      for await (const opusBuffer of opusAudioStream) {
        await this.processAudio(opusBuffer);
      }
    } catch (error) {
   ...
   */
  private async processAudio(opusBuffer: Buffer): Promise<void> {
    try {
      const pcm16k = this.decoder.decode(opusBuffer);
      const pcm24k = await resample16kTo24kMono(pcm16k);
      this.client.appendInputAudio(pcm24k);
    } catch (error) {
